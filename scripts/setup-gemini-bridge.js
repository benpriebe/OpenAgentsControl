      ...
      if (entry.isDirectory()) {
        ...
        if (match) {
          ...
          if (!yamlBlock.includes('name:')) {
            ...
            console.log(`  ✓ Added 'name: ${commandName}' frontmatter in-memory for command ${entry.name}`);
          }
        } else {
          content = `---\nname: ${commandName}\ndescription: OAC command converted to Antigravity skill\n---\n\n${content}`;
          console.log(`  ✓ Added default frontmatter in-memory for command ${entry.name}`);
        }
        
        // Write the modified content to the local skill file (pristine/gitignored)
      ...
      if (entry.isDirectory()) {
        ...
        if (match) {
          ...
          if (updatedYaml !== yamlBlock) {
            content = content.replace(frontmatterRegex, `---\n${updatedYaml}\n---`);
            console.log(`  ✓ Updated frontmatter in-memory for agent ${entry.name}`);
          }
        }
        ...
        // Write the modified content directly to the local agent file (pristine/gitignored)
        fs.writeFileSync(destAgentFile, content, 'utf8');
